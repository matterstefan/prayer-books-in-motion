#!/usr/bin/env python3
"""Transform the exploratory MEI JSON sample into linked CSV tables.
...
SOURCE_PATH = REPOSITORY_ROOT / "data" / "sample" / "mei-api-sample.json"
...
def main() -> None:
    ...
    for relationship in relationships:
        ...
        if mei_id not in set(record_ids):
        ...
        copy_rows.append(
            {
                ...
                "source_retrieved_at": text(metadata.get("retrieved_at")),
    ...
    print(f"MEI copies: {len(copy_rows)}")
    print(f"Provenance blocks: {len(provenance_rows)}")
    print(f"Place occurrences: {len(place_rows)}")
    print(f"ISTC–MEI relationships: {len(relationship_rows)}")
